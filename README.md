...
# Context Aware Emotional Embodied Agents 

Based on Generative Agents: Interactive Simulacra of Human Behavior 



This repository accompanies our research paper titles Context Aware Emotional Embodied Agents. The agents are based on the papper titled "[Generative Agents: Interactive Simulacra of Human Behavior](https://arxiv.org/abs/2304.03442)." It contains our core simulation module for  generative agents—computational agents that simulate believable human behaviors—and their game environment. Below, we document the steps for setting up the simulation environment on your local machine and for replaying the simulation as a demo animation. Also contains the instructions for running the Embodied agents on a navel robot
...
Open up another command line (the one you used in Step 1 should still be running the environment server, so leave that as it is). Navigate to the root, where the reverie and environment folders are.

    python reverie/backend_server/main.py
...
### Step 6. Running in a Navel Robot
Firtly, replace de cred.json file in the navel_sripts/NAVEL folder with your google developer speech-to-text api credentials file

In the environment/frontend_server/robot.py change the HOST vairable to your current IP address, and copy this same address to your navel robot navel_behavior.py file in the navel scripts

follow the https://doc.navelrobotics.com/ documentation to run the navel_behavior.py file in your navel robot

run the reverie main file like before, with the option:

run robot

do not reset the agents unless you want a completely empty agent for some reason.

choose from the available personas with the indexes from the provided list

If you want to use previously generated memories:

choose emotional agents, and the statrting simulation emotional_boys_18
choosed non emotional agents and the sim non_emotional_boys
