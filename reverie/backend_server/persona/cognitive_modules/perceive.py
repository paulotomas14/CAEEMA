...
def perceive(persona, maze, personas): 
  ...
  '''
  latest_events_sum = persona.a_mem.get_summarized_latest_events(
                                    persona.scratch.retention)
  latest_events = persona.a_mem.get_latest_events(
                                    persona.scratch.retention)
                                    '''
  ...
  for p_event in perceived_events:
    ...
    p_event = (s, p, o, dist)
    p_events.append((s,p,o,desc, dist))
    p_events_sum.append((s,p,o, dist))
    ...
    if p_event not in latest_events_sum:
      ...
      #print("P EVENT =  "+ str(p_event))
      node = persona.a_mem.add_event(persona.scratch.curr_time, None,
                           s, p, o, desc, keywords, event_poignancy, 
                           event_embedding_pair, chat_node_ids, dist)
      #print("NODE DIST = " + str(node.dist))
      
      ret_events.append(node)
      ret_events_sum.append((node.subject, node.predicate, node.object, node.dist))
      persona.scratch.importance_trigger_curr -= event_poignancy
      persona.scratch.importance_ele_n += 1

  new_events, ongoing_events, finished_events = [], [], []

  if persona.emotional:

    new_events = [p_event for p_event in p_events if (p_event[0], p_event[1], p_event[2], p_event[3]) in ret_events_sum ]
    #and p_event[2]!="idle"
    ongoing_events = [p_event for p_event in p_events if (p_event[0], p_event[1], p_event[2], p_event[3]) in latest_events_sum ]
    #and p_event[2]!="idle"
    finished_events = [p_event for p_event in latest_events if (p_event[0], p_event[1], p_event[2], p_event[3]) not in p_events_sum]
      #and p_event[2]!="idle"
  ...
  events = (ret_events, new_events, ongoing_events, finished_events)
  #print("NEW EVENTS!" + str(events))
  return events
