...
from persona.cognitive_modules.variables import verbose_reflection
from utils import *
...
def reflect(persona):
  ...
  #print (Fore.GREEN + persona.scratch.name, "al;sdhfjlsad", persona.scratch.chatting_end_time)
  #print(Style.RESET_ALL)
  #print("DEBUG", persona.scratch.curr_time + datetime.timedelta(seconds=SECS_PER_STEP))
  if (
    (persona.scratch.chatting_end_time 
      and persona.scratch.curr_time + datetime.timedelta(seconds=SECS_PER_STEP) >= persona.scratch.chatting_end_time) 
      or (persona.scratch.chatting_end_time is None and persona.scratch.chatting_with)
      ): 
    ...
    if persona.scratch.chatting_with:
      if persona.a_mem.get_last_chat(persona.scratch.chatting_with):
        ev = persona.a_mem.get_last_chat(persona.scratch.chatting_with).node_id
      else:
        ev = ["...", "..."]
    ...
    '''persona.scratch.previously_chatting_with = persona.scratch.chatting_with
    persona.scracth.previously_chatting_with.update(
        {
          persona.scratch.chatting_with:
          { "time":persona.scratch.curr_time,"chat": persona.scratch.chat }
        }
      )'''
