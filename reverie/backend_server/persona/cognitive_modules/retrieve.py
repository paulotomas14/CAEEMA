...
def retrieve(persona, perceived): 
  ...
  for event in perceived:
    ##print("EVENT in perceived = " + str(event)) 
    ...
    if persona.emotional:
      ##print("RETRIVE INTENSITY")
      intensity = persona.emotional_layer.retrieve_intensity(event)
    else:
      if "is idle" in event.description:
        intensity = 5
      else:
        if persona.name in event.subject:
          intensity = 8
        else:
          intensity = 10

    retrieved[event.description]["intensity"] = float(intensity)
    retrieved[event.description]["distance"] = float(event.dist)
...
def new_retrieve(persona, focal_points, n_count=30): 
  ...
  for focal_pt in focal_points: 
    ...
    nodes = [[i.last_accessed, i]
              ...
              if "idle" not in i.embedding_key and "blank" not in i.embedding_key]
